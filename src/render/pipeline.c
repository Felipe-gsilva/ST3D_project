...
void createImageViews(App *pApp) {
  ...
  for (size_t i = 0; i < pApp->swapChainImageCount; i++) {
    ...
    if (vkCreateImageView(pApp->logicalDevice, &createInfo, NULL,
...
VkShaderModule createShaderModule(App *pApp, ShaderFile *shaderFile) {
  ...
  if (vkCreateShaderModule(pApp->logicalDevice, &createInfo, NULL,
...
void createGraphicsPipeline(App *pApp) {
  ...
  pApp->shaderCount = 2;
  pApp->shaderStages =
      malloc(sizeof(VkPipelineShaderStageCreateInfo) * pApp->shaderCount);
  ...
  if (vkCreatePipelineLayout(pApp->logicalDevice, &pipelineLayoutInfo, NULL,
  ...
  if (vkCreateRenderPass(pApp->logicalDevice, &renderPassInfo, NULL,
  ...
  if (vkCreateGraphicsPipelines(pApp->logicalDevice, VK_NULL_HANDLE, 1,
                                &pipelineInfo, NULL,
...
void cleanupShaders(App *pApp) {
  for (size_t i = 0; i < pApp->shaderCount; i++) {
    vkDestroyShaderModule(pApp->logicalDevice, pApp->shaderStages[i].module,
                          NULL);
  }

...
u32 clamp(u32 val, u32 min, u32 max) { return max(min, min(max, val)); }

VkSurfaceFormatKHR
chooseSwapSurfaceFormat(u32 formatCount, VkSurfaceFormatKHR *availableFormats) {
...
VkPresentModeKHR
chooseSwapPresentMode(u32 presentModeCount,
...
void createSwapChain(App *pApp) {
  ...
  VkSurfaceFormatKHR surfaceFormat = chooseSwapSurfaceFormat(
      swapChainSupport.formatCount, swapChainSupport.formats);
  VkPresentModeKHR presentMode = chooseSwapPresentMode(
      swapChainSupport.presentModeCount, swapChainSupport.presentModes);
  VkExtent2D extent =
      chooseSwapExtent(pApp->window, swapChainSupport.capabilities);
  ...
  if (vkCreateSwapchainKHR(pApp->logicalDevice, &createInfo, NULL,
                           &pApp->swapChain) != VK_SUCCESS) {
    printf("Failed to create Swap Chain!\n");
    exit(5);
  }

  vkGetSwapchainImagesKHR(pApp->logicalDevice, pApp->swapChain, &imageCount,
                          NULL);
...
void cleanupSwapChain(App *pApp) {
  ...
  vkDestroySwapchainKHR(pApp->logicalDevice, pApp->swapChain, NULL);

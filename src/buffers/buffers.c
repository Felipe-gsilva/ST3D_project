...
void createCommandPool(App *pApp) {
  ...
  if (vkCreateCommandPool(pApp->logicalDevice, &poolInfo, NULL,
                          &pApp->commandPool) != VK_SUCCESS) {
...
void createFramebuffers(App *pApp) {
  ...
  pApp->swapChainFramebuffers =
      (VkFramebuffer *)malloc(sizeof(VkFramebuffer) * swapChainImageCount);
  ...
  for (size_t i = 0; i < swapChainImageCount; i++) {
    ...
    if (vkCreateFramebuffer(pApp->logicalDevice, &framebufferInfo, NULL,
...
void cleanupFramebuffers(App *pApp) {
  for (size_t i = 0; i < pApp->swapChainImageCount; i++) {
    vkDestroyFramebuffer(pApp->logicalDevice, pApp->swapChainFramebuffers[i],
                         NULL);
  }
...
void cleanupCommandbuffer(App *pApp) {
  vkFreeCommandBuffers(pApp->logicalDevice, pApp->commandPool,
                       (u32)MAX_FRAMES_IN_FLIGHT, pApp->commandBuffer);
}
...
void recordCommandBuffer(App *pApp, VkCommandBuffer commandBuffer,
  ...
  VkRect2D scissor = {
      .offset.x = 0, .offset.y = 0, .extent = pApp->swapChainExtent};
  ...
  vkCmdDraw(commandBuffer, (sizeof(vertexBuffers) / sizeof(vertexBuffers[0])),
            1, 0, 0);
...
void createCommandBuffer(App *pApp) {
  ...
  pApp->commandBuffer = (VkCommandBuffer *)malloc(sizeof(VkCommandBuffer) *
  ...
  if (vkAllocateCommandBuffers(pApp->logicalDevice, &allocInfo,
                               pApp->commandBuffer) != VK_SUCCESS) {
...
void createSyncObjects(App *pApp) {
  ...
  pApp->imageAvailableSemaphore =
      (VkSemaphore *)calloc(sizeof(VkSemaphore), MAX_FRAMES_IN_FLIGHT);
  pApp->renderFinishedSemaphore =
      (VkSemaphore *)calloc(sizeof(VkSemaphore), MAX_FRAMES_IN_FLIGHT);
  pApp->inFlightFence =
      (VkFence *)calloc(sizeof(VkFence), (u32)MAX_FRAMES_IN_FLIGHT);
  ...
  for (int i = 0; i < (int)MAX_FRAMES_IN_FLIGHT; i++) {
    if (vkCreateSemaphore(pApp->logicalDevice, &semaphoreInfo, NULL,
    ...
    if (vkCreateFence(pApp->logicalDevice, &fenceInfo, NULL,
...
void cleanupSyncObjects(App *pApp) {
  for (int i = 0; i < (int)MAX_FRAMES_IN_FLIGHT; i++) {
    vkDestroySemaphore(pApp->logicalDevice, pApp->renderFinishedSemaphore[i],
                       NULL);
    vkDestroySemaphore(pApp->logicalDevice, pApp->imageAvailableSemaphore[i],
                       NULL);

...
void createVertexBuffer(App *pApp) {
  ...
  if (vkCreateBuffer(pApp->logicalDevice, &bufferInfo, NULL,
                     &pApp->vertexBuffer) != VK_SUCCESS) {
    printf("Failed to create vertex buffer\n");
    exit(1);
  }

  VkMemoryRequirements memRequirements;
  vkGetBufferMemoryRequirements(pApp->logicalDevice, pApp->vertexBuffer,
                                &memRequirements);
  ...
  VkMemoryAllocateInfo allocInfo = {
      ...
      .memoryTypeIndex =
          findMemoryType(memRequirements.memoryTypeBits,
                         ...
                         pApp)};
  if (vkAllocateMemory(pApp->logicalDevice, &allocInfo, NULL,
                       &pApp->vertexBufferMemory) != VK_SUCCESS) {
    printf("Failed to allocate vertex buffer memory\n");
    exit(1);
  }

  vkBindBufferMemory(pApp->logicalDevice, pApp->vertexBuffer,
                     pApp->vertexBufferMemory, 0);

  void *data;
  vkMapMemory(pApp->logicalDevice, pApp->vertexBufferMemory, 0, bufferInfo.size,
              0, &data);
